...
kotlin {
    ...
    val xcf = XCFramework("ProcMacro")

    macosArm64 {
        binaries.framework { baseName = "ProcMacro"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework { baseName = "ProcMacro"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "ProcMacro"
        flattenPackage = "io.github.kotlinmania.procmacro"
    }

    android {
        namespace = "io.github.kotlinmania.procmacro"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "proc-macro-kotlin", version.toString())

    pom {
        name.set("proc-macro-kotlin")
        description.set("Kotlin Multiplatform port of Rust's compiler-internal proc_macro crate, backed by JetBrains' multiplatform KotlinLexer / KtTokens / KotlinParser. The Compiler-variant counterpart to proc-macro2-kotlin's Fallback backend.")
...
// Make `build` compile AND LINK every configured target, including the
// native test binaries the default Kotlin/Native graph leaves off the
// `build` lifecycle. The default Gradle `build` produces compile outputs
// for every target but only links debug test binaries for a few
// host-runnable ones, which lets non-host native targets (device iOS,
// device tvOS/watchOS, Android Native, Linux ARM64) silently regress.
// The explicit set below is the audit contract; the late dynamic matcher
// is a safety net for any new generated *MainClasses / *TestClasses /
// *Binaries / *XCFramework tasks introduced after this file is read.
