...
// Exact test lifecycle task. Runs the required host tests and Swift Export smoke test.
tasks.register("test") {
    group = "verification"
...
// Ensure the SPM Package.swift generated by Kotlin's embedSwiftExportForXcode
// declares platforms: [.macOS(.v14)], so swift test doesn't fail with
// "requires macos 14.0 or later" under Swift 6.
tasks.matching { it.name.contains("GenerateSPMPackage") }.configureEach {
    doLast {
        val packageSwiftFiles =
            fileTree(layout.buildDirectory.dir("SPMPackage")) {
                include("**/Package.swift")
            }
        for (pkg in packageSwiftFiles) {
    ...
    doLast {
        ...
        val swiftBuildDir =
            layout.buildDirectory
                ...
                .asFile
                .absolutePath
        val swiftBuildFile = File(swiftBuildDir)
