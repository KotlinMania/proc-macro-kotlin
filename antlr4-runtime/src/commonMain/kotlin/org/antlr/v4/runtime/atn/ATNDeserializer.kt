 ...
 * @author Sam Harwell
 */
class ATNDeserializer @kotlin.jvm.JvmOverloads constructor(deserializationOptions: ATNDeserializationOptions? = ATNDeserializationOptions.defaultOptions) {
...
//					   " "+arg1+","+arg2+","+arg3);
    ...
    private fun deserializeSets(data: IntArray, p: Int, sets: List<IntervalSet?>): Int {
        var p = p
        val nsets = data[p++]
        for (i in 0..<nsets) {
    ...
    protected fun checkCondition(condition: Boolean, message: String? = null) {
        if (!condition) {
    ...
    protected fun stateFactory(type: Int, ruleIndex: Int): ATNState? {
        val s: ATNState?
        when (type) {
    ...
    companion object {
        ...
        protected fun toInt(c: Char): Int {
            return c.code
        }

        protected fun toInt32(data: CharArray, offset: Int): Int {
            return data[offset].code or (data[offset + 1].code shl 16)
        }

        protected fun toInt32(data: IntArray, offset: Int): Int {
            return data[offset] or (data[offset + 1] shl 16)
        }
        ...
        fun decodeIntsEncodedAs16BitWords(data16: CharArray, trimToSize: Boolean = false): IntArray {
