 ...
 */
class UnbufferedCharStream @kotlin.jvm.JvmOverloads constructor(bufferSize: Int = 256) : CharStream {
    /**
    ...
    protected var input: Reader? = null
    ...
    constructor(input: InputStream?) : this(input, 256)

    constructor(input: Reader) : this(input, 256)

    constructor(input: InputStream?, bufferSize: Int) : this(input, bufferSize, StandardCharsets.UTF_8)

    constructor(input: InputStream?, bufferSize: Int, charset: Charset?) : this(bufferSize) {
        this.input = InputStreamReader(input, charset)
        fill(1) // prime
    }

    constructor(input: Reader, bufferSize: Int) : this(bufferSize) {
        this.input = input
        fill(1) // prime
    }
    fun consume() {
    ...
    fun seek(index: Int) {
        ...
        if (index > currentCharIndex) {
            ...
            index = minOf(index, this.bufferStartIndex + n - 1)
        }

        // index == to bufferStartIndex should set p to 0
        val i = index - this.bufferStartIndex
        require(i >= 0) { "cannot seek to negative index " + index }
        if (i >= n) {
            throw UnsupportedOperationException(
                "seek to index outside buffer: " +
                        index + " not in " + this.bufferStartIndex + ".." + (this.bufferStartIndex + n)
            )
        }
