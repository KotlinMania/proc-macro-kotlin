...
internal data class Utf8Chunk(val valid: String, val invalid: ByteArray)
 ...
 */
internal fun utf8Chunks(bytes: ByteArray): Sequence<Utf8Chunk> = sequence {
    var i = 0
    while (i < bytes.size) {
        val validStart = i
        while (i < bytes.size) {
 ...
 */
internal fun utf8SequenceLength(bytes: ByteArray, offset: Int): Int {
 ...
 */
internal fun unicodeScalars(s: String): Sequence<Int> = sequence {
    var i = 0
    while (i < s.length) {
        val c = s[i]
