...
internal class ASTMarkersImpl private constructor(
    ...
    override fun kind(i: Int): MarkerKind = packer.kind(i)
    ...
    fun hasError(i: Int): Boolean = packer.hasErrors(i)

    fun id(i: Int): Int = packer.id(i)

    override fun lexemeCount(i: Int): Int = packer.lexemeInfo(i).count

    override fun lexemeRelOffset(i: Int): Int = packer.lexemeInfo(i).relOffset

    override fun collapsed(i: Int): Boolean = packer.collapsed(i)

    override fun markersCount(i: Int): Int = packer.markersCount(i)

    override fun elementType(i: Int): SyntaxElementType = elementTypes[i]

    override fun chameleonAt(lexemeIndex: Int): ChameleonRef = chameleonsMap[lexemeIndex]!!

    override fun chameleons(): List<ChameleonChange> =
        chameleonsMap.entries
            .asSequence()
            .map { (key, value) -> ChameleonChange(key, value) }
            .toList()

    private fun substituteImpl(
        astMarkers: ASTMarkers,
        i: Int,
        lexemeIndex: Int,
    ) {
    ...
    private fun removeMarkersFromMaps(
        start: Int,
        end: Int,
    ) {
    ...
    private fun renumber(
        ...
        (0 until size).forEach { index ->
            ...
            val newId =
                renumberMap.computeIfAbsent(id(index)) { oldId ->
    ...
    private class Packer {
        ...
        fun collapsed(i: Int): Boolean = ints[index(i)] and 4 == 4

        fun hasErrors(i: Int): Boolean = ints[index(i)] and 8 == 8
    ...
    private inner class MutableContextImpl : ASTMarkers.MutableContext {
        ...
        override fun changeChameleons(changes: List<ChameleonChange>) {
            ast = ast.withChameleons(changes)
        }

        override fun changeLexCount(
            startMarker: Int,
            endMarker: Int,
            lexCount: Int,
        ) {
        ...
        override fun changeMarkerCount(
            startMarker: Int,
            endMarker: Int,
            markerCount: Int,
        ) {
    ...
    private fun withChameleons(changes: List<ChameleonChange>): ASTMarkersImpl =
        ASTMarkersImpl(
            packer = packer,
            elementTypes = elementTypes,
            errorMessages = errorMessages,
            chameleonsMap =
                Int2ObjectOpenHashMap<ChameleonRef>().apply {
                    changes.forEach { change ->
                        put(change.lexemeIndex, change.ref)
                    }
                },
            nextId = nextId,
        )
