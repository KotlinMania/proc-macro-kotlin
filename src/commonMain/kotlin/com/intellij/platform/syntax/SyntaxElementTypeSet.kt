...
internal fun syntaxElementTypeSetOf(vararg tokens: SyntaxElementType): SyntaxElementTypeSet =
...
class SyntaxElementTypeSet internal constructor(
    ...
    fun containsNullable(element: SyntaxElementType?): Boolean = element != null && contains(element)

    override fun contains(element: SyntaxElementType): Boolean = bitSet.contains(element.index)

    override fun containsAll(elements: Collection<SyntaxElementType>): Boolean = elements.all { contains(it) }

    override fun isEmpty(): Boolean = bitSet.isEmpty()

    override fun iterator(): Iterator<SyntaxElementType> = tokens.iterator()
...
internal fun flattenSyntaxElementTypeSets(vararg sets: SyntaxElementTypeSet): SyntaxElementTypeSet =
    sets.asList().flatten().asSyntaxElementTypeSet()
