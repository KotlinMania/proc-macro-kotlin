...
import com.intellij.platform.syntax.parser.ProductionList
...
import com.intellij.platform.syntax.parser.SyntaxElementTypeList
import com.intellij.platform.syntax.parser.SyntaxElementTypeRemapper
import com.intellij.platform.syntax.parser.SyntaxElementTypeSink
...
internal class SyntaxTreeBuilderImpl(
    ...
    override val lastDoneMarker: SyntaxTreeBuilder.Marker?
        get() =
            ((myProduction.size - 1) downTo 0).firstNotNullOfOrNull { index ->
    ...
    override fun isWhitespaceOrComment(elementType: SyntaxElementType): Boolean =
        myWhitespaces.contains(elementType) || myComments.contains(elementType)
    ...
    fun lexType(index: Int): SyntaxElementType =
        if (myLexTypes != null) myLexTypes[index + lexStartIndex] else tokens.getTokenType(index)!!

    fun lexStart(index: Int): Int =
        if (myLexStarts != null) myLexStarts[index + lexStartIndex] else tokens.getTokenStart(index)

    fun remap(
        index: Int,
        value: SyntaxElementType,
    ) {
    ...
    internal fun processDone(
        ...
        if (whitespaceOrCommentBindingPolicy.isLeftBound(marker.getNodeType()) &&
            isEmpty(marker.getStartTokenIndex(), doneLexeme)
        ) {
    ...
    private fun isEmpty(
        ...
        (startIdx..<endIdx).all { i ->
            isWhitespaceOrComment(lexType(i))
        }

    private fun doValidityChecks(
        marker: CompositeMarker,
        before: CompositeMarker?,
    ) {
        if (marker.isDone) {
    ...
    fun prepareProduction(): ProductionResult {
        ...
        if (myCurrentLexeme < lexemeCount) {
            ...
            logger.errorWithAttachments(
                "Tokens $missed were not inserted into the tree. ${language ?: ""}",
                listOf(Attachment("missedTokensFragment.txt", text.toString())),
            )
        }
        ...
        if (rootMarker.getEndTokenIndex() < lexemeCount) {
            ...
            result.copyTokenTypesTo(
                object : SyntaxElementTypeSink {
                    override fun set(
                        index: Int,
                        value: SyntaxElementType,
                    ) {
                ...
                rootMarker.getEndTokenIndex(),
                0,
                missed.size,
            )
            logger.errorWithAttachments(
                "Tokens ${missed.contentToString()} are outside of root element \"${rootMarker.getNodeType()}\".",
                listOf(Attachment("outsideTokensFragment.txt", text.toString())),
            )
        }
    ...
    inner class ProductionResultImpl : ProductionResult {
        ...
        override fun copyTokenStartsToArray(
            dest: IntArray,
            srcStart: Int,
            destStart: Int,
            length: Int,
        ...
        override fun copyTokenTypesTo(
            dest: SyntaxElementTypeSink,
            srcStart: Int,
            destStart: Int,
            length: Int,
        ) {
            if (myLexTypes != null) {
                for (i in 0 until length) {
                    dest.set(destStart + i, myLexTypes[srcStart + lexStartIndex + i])
                }
            } else {
                for (i in 0 until length) {
                    dest.set(destStart + i, tokens.getTokenType(srcStart + i)!!)
                }
            }
        }
    }
    ...
    private inner class WhitespaceBalancer {
        ...
        private fun WhitespacesAndCommentsBinder.inferNewLexemeIndex(
            ...
            val edgePosition = this.getEdgePosition(SyntaxElementTypeList(wsTokens), atEnd, tokenTextGetter)
         ...
         */
        private fun getWhitespaceStartIndex(
            lexemeIndex: Int,
            prevProductionLexIndex: Int,
        ): Int {
        ...
        private fun getPrevProductionLexIndex(
            i: Int,
            isBinderRecursive: Boolean,
        ): Int {
    ...
    private inner class RelativeTokenTypesView(
        ...
        override fun get(index: Int): SyntaxElementType = lexType(myStart + index)
    }
    ...
    private inner class RelativeTokenTextView(
        ...
        override fun get(i: Int): CharSequence =
            CharSequenceSubSequence(text, lexStart(myStart + i), lexStart(myStart + i + 1))
    }
}

private const val UNBALANCED_MESSAGE: String =
