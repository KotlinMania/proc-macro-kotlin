...
internal class CompositeMarker(
    ...
    override fun getEndOffset(): Int = builder.lexStart(endIndex) + builder.startOffset
    ...
    override fun done(type: SyntaxElementType) {
        if (type === SyntaxTokenTypes.ERROR_ELEMENT) {
            builder.logger.warn(
                "Error elements with empty message are discouraged. Please use builder.error() instead",
                RuntimeException(),
            )
        }
    ...
    override fun doneBefore(
        ...
        if (type === SyntaxTokenTypes.ERROR_ELEMENT) {
            builder.logger.warn(
                "Error elements with empty message are discouraged. Please use builder.errorBefore() instead",
                RuntimeException(),
            )
        }
        this@CompositeMarker.type = type
    ...
    override fun getLexemeIndex(done: Boolean): Int = if (done) endIndex else startIndex
