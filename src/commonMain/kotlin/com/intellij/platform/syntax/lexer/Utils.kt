...
internal class TokenListBuilder internal constructor() {
    ...
    internal fun build(): TokenList {
        ...
        val lexTypes = Array(tokens.size) { tokens[it].second }
        return TokenList(lexStarts, lexTypes, tokens.size, tokenizedText)
    }
}
...
private data object EmptyTokenList : TokenList {
    ...
    override fun slice(
        ...
        if (start == 0 && end == 0) {
            ...
            error("Token list is empty. You may only call `slice(0,0)`.")
        }

    override fun remap(
        index: Int,
        newValue: SyntaxElementType,
    ) = error("Can't remap in an empty token list.")
}

internal fun buildTokenList(builderFunc: TokenListBuilder.() -> Unit): TokenList =
