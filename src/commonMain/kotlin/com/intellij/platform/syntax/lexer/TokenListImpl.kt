...
internal class TokenListImpl(
    ...
    fun assertMatches(
        ...
        for (j in 0 until tokenCount) {
            if (sequence.lexStarts[j] != lexStarts[startIndex + j] ||
                sequence.lexTypes[j] !== lexTypes[startIndex + j]
            ) {
                check(false)
            }
        }
    ...
    override fun getTokenEnd(index: Int): Int = lexStarts[startIndex + index + 1]
}

private fun emptyTokenList(): TokenListImpl = TokenListImpl(IntArray(1), emptyArray(), 0, "")
...
internal class TokenListLexerImpl(
    ...
    fun startMeasured(
        ...
        if (startDuration.inWholeMilliseconds > LEXER_START_THRESHOLD) {
            logger.debug(
                "Starting lexer took: $startDuration; at $startOffset - $endOffset; state: $initialState; text: ${buffer.shortenTextWithEllipsis(
                    1024,
                    500,
                )}",
            )
    ...
    override fun getTokenType(): SyntaxElementType? = tokens.getTokenType(getState())

    override fun getTokenStart(): Int = tokens.getTokenStart(getState())

    override fun getTokenEnd(): Int = tokens.getTokenEnd(getState())

    override fun advance() {
        state++
    }

    override fun getBufferSequence(): CharSequence = tokens.tokenizedText

    override fun getBufferEnd(): Int = tokens.tokenizedText.length
...
internal class Builder(
    ...
    fun performLexing(): TokenListImpl {
        ...
        val tokenTypes =
            Array(i) { index ->
                lexTypes[index] ?: error("Missing token type at index $index")
            }
        return TokenListImpl(lexStarts, tokenTypes, i, text)
    }

    private fun reportDescendingOffsets(
        tokenIndex: Int,
        offset: Int,
        tokenStart: Int,
    ) {
        ...
        sb
            .append("Token sequence broken")
            .append(
                "\n  this: '",
            ).append(lexer.getTokenText())
            .append("' (")
            .append(tokenType)
            .append(") ")
            .append(tokenStart)
        ...
        if (tokenIndex > 0) {
            ...
            sb
                .append(
                    ...
                    text.subSequence(prevStart, offset),
                ).append("' (")
                .append(lexTypes[tokenIndex - 1])
                .append(") ")
                .append(prevStart)
                .append(":")
                .append(offset)
        ...
        sb
            ...
            .append(quoteEnd)
            .append("] '")
            .append(text.subSequence(quoteStart, quoteEnd))
            .append('\'')
