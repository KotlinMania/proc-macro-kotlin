...
object HashCommon {
     ...
     */
    fun invMix(x: Int): Int = (x xor (x ushr 16)) * INV_INT_PHI
     ...
     */
    fun float2int(f: Float): Int = f.toRawBits()
     ...
     */
    fun long2int(l: Long): Int = (l xor (l ushr 32)).toInt()
     ...
     */
    fun nextPowerOfTwo(x: Int): Int = 1 shl (32 - (x - 1).countLeadingZeroBits())
     ...
     * @return the least power of two greater than or equal to the specified value.
     */
    fun nextPowerOfTwo(x: Long): Long = 1L shl (64 - (x - 1).countLeadingZeroBits())
    ...
    fun arraySize(
        ...
        if (s >
            ...
            throw IllegalArgumentException("Too large ($expected expected elements with load factor $f)")
        }
        return s.toInt()
    }
    ...
    fun bigArraySize(
        expected: Long,
        f: Float,
    ): Long = nextPowerOfTwo(ceil((expected / f).toDouble()).toLong())
}
