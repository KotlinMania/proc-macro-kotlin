...
internal inline fun IntList.forEach(action: (Int) -> Unit) {
    for (index in indices) action(get(index))
}

internal fun IntList.contains(element: Int): Boolean = indexOf(element) != -1

fun IntList.isEmpty(): Boolean = size == 0

internal fun IntList.isNotEmpty(): Boolean = size != 0
...
internal fun IntList.length(): Int = size
