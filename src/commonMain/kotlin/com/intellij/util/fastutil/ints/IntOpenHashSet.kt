...
class IntOpenHashSet(
    ...
    private fun realSize(): Int = if (containsNull) size - 1 else size
    ...
    fun addAll(elements: IntList): Boolean {
        if (f <= .5) {
            ensureCapacity(elements.size) // The resulting collection will be sized for c.size() elements
        } else {
            tryCapacity((size + elements.size).toLong()) // The resulting collection will be tentatively sized for size() + c.size() elements
        }
    ...
    override fun add(key: Int): Boolean {
        ...
        if (key == 0) {
            ...
            var curr: Int
            val keys = this.key // The starting point.
            if (keys[
                ...
                while (keys[((pos + 1) and mask).also { pos = it }].also {
                        curr = it
                    } != 0
                ) {
                    if (curr == key) return false
    ...
    override fun contains(value: Int): Boolean {
        if (value == 0) return containsNull
        var curr: Int
        val key = this.key
        var pos: Int // The starting point.
        if (key[
                (HashCommon.mix(value) and mask).also {
                    pos = it
                },
            ].also { curr = it } == 0
        ) {
   ...
   */
    ...
    private fun rehash(newN: Int) {
        ...
        while (j-- != 0) {
            ...
            if (newKey[
                    (HashCommon.mix(key[i]) and mask).also {
                        pos = it
                    },
                ] != 0
            ) {
    ...
    companion object {
         ...
         */
        fun of(): IntOpenHashSet = IntOpenHashSet()
         ...
         */
        internal fun of(vararg a: Int): IntOpenHashSet {
