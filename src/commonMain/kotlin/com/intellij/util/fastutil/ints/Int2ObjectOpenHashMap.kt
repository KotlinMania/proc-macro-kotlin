 ...
 * @see HashCommon
 */
internal class Int2ObjectOpenHashMap<V> : MutableIntMap<V> {
    /** The array of keys.  */
    private var key: IntArray

    /** The array of values.  */
    private var value: MutableList<V?>
    ...
    constructor(k: IntArrayList, v: Array<V>, f: Float = Hash.DEFAULT_LOAD_FACTOR) : this(k.size, f) {
        if (k.size !=
            ...
            throw IllegalArgumentException(
                "The key array and the value array have different lengths (" + k.size + " and " + v.size + ")",
            )
        }
    ...
    fun putAll(from: Int2ObjectOpenHashMap<V>) {
        if (f <= .5) {
            ensureCapacity(from.size) // The resulting map will be sized for m.size() elements
        } else {
            tryCapacity((size + from.size).toLong()) // The resulting map will be tentatively sized for size() + m.size() elements
        }
    ...
    private fun find(k: Int): Int {
        ...
        if (key[
                (HashCommon.mix(k) and mask).also {
                    pos = it
                },
            ].also { curr = it } == 0
        ) {
    ...
    override fun remove(key: Int): V? {
        ...
        var pos: Int // The starting point.
        if (keys[
                (HashCommon.mix(key) and mask).also {
                    pos = it
                },
            ].also { curr = it } == 0
        ) {
        ...
        while (true) {
            if (keys[((pos + 1) and mask).also { pos = it }].also {
                    curr = it
                } == 0
            ) {
                return defaultReturnValue()
    ...
    override operator fun get(key: Int): V? {
        if (key == 0) return if (containsNullKey) value[n] else defaultReturnValue()
        var curr: Int
        val keys = this.key
        var pos: Int // The starting point.
        if (keys[
        ...
        if (key == curr) return value[pos] // There's always an unused entry.
        while (true) {
            if (keys[((pos + 1) and mask).also { pos = it }].also {
                    curr = it
                } == 0
    ...
    fun defaultReturnValue(): V? = defaultValue
    ...
    private abstract inner class MapIterator {
        ...
        private fun shiftKeys(pos: Int) { // Shift entries with the same hash.
            ...
            var curr: Int
            val key = this@Int2ObjectOpenHashMap.key
            val value = this@Int2ObjectOpenHashMap.value
            while (true) {
                pos = (pos.also { last = it } + 1) and mask
                while (true) {
                    if ((key[pos].also { curr = it }) == 0) {
    ...
    private inner class KeyIterator :
        MapIterator(),
        IntIterator {
        override fun next(): Int = key[nextEntry()]
    }
    ...
    private inner class ValueIterator :
        MapIterator(),
        Iterator<V> {
        override fun next(): V = value[nextEntry()]!!
    }
    ...
    private fun rehash(newN: Int) {
        ...
        val newValue = MutableList<V?>(newN + 1) { null }
        var i = n
        var pos: Int
        var j = realSize()
        while (j-- != 0) {
            ...
            if (newKey[(HashCommon.mix(key[i]) and mask).also { pos = it }] !=
                0
            ) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is Int2ObjectOpenHashMap<*> && contentEquals(other)
    }

    internal fun contentEquals(other: Int2ObjectOpenHashMap<*>): Boolean {
