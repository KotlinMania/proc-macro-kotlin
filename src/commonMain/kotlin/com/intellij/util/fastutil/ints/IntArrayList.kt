...
class IntArrayList :
    ...
    private constructor(
        a: IntArray,
        @Suppress("unused") wrapped: Boolean,
    ) {
        this.a = a
    }
     ...
     */
    constructor(i: IntIterator) : this() {
    ...
    fun ensureCapacity(capacity: Int) {
        if (capacity <= a.size ||
            (a.contentEquals(IntArrays.DEFAULT_EMPTY_ARRAY) && capacity <= DEFAULT_INITIAL_CAPACITY)
        ) {
            return
        }
    ...
    private fun grow(capacity: Int) {
        ...
        if (!a.contentEquals(IntArrays.DEFAULT_EMPTY_ARRAY)) {
            capacity =
                max(min((a.size + (a.size shr 1)), Arrays.MAX_ARRAY_SIZE), capacity)
    ...
    override fun addElements(
        ...
        this.a.copyInto(
            destination = this.a,
            destinationOffset = index + length,
            startIndex = index,
            endIndex =
                index + (size - index),
        )
    ...
    override fun equals(other: Any?): Boolean = other === this || (other is IntList && compareTo(other) == 0)
     ...
     */
    override fun compareTo(other: IntList): Int {
        val s1 = length()
        val s2 = other.length()
        var e1: Int
        var e2: Int
    ...
    companion object {
        ...
        fun wrap(a: IntArray): IntArrayList = wrap(a, a.size)
        ...
        fun wrap(
            ...
            if (length >
                ...
                throw IllegalArgumentException(
                    "The specified length (" + length + ") is greater than the array size (" + a.size + ")",
                )
            }
        ...
        fun of(): IntArrayList = IntArrayList()

        internal fun of(vararg init: Int): IntArrayList = wrap(init)
    }
}
