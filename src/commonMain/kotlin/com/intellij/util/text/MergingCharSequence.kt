...
class MergingCharSequence(
    private val s1: CharSequence,
    private val s2: CharSequence,
) : CharSequence {
    ...
    override fun subSequence(
        ...
        when {
            ...
            startIndex >= s1.length && endIndex >= s1.length ->
                s2.subSequence(
                    startIndex - s1.length,
                    endIndex - s1.length,
                )
            else -> MergingCharSequence(s1.subSequence(startIndex, s1.length), s2.subSequence(0, endIndex - s1.length))
        }

    override fun toString(): String = s1.toString() + s2.toString()
}
